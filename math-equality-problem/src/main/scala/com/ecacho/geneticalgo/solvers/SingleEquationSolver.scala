...
class SingleEquationSolver(equation: MathEquation) extends GeneticAlgorithm {
...
object SingleEquationSolver extends App {
  val expressions = Seq(
  ...
  expressions.foreach(myexpr => {
    ...
    val solver = new SingleEquationSolver(equation)
